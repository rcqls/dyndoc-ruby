    ...
    if i=(dyn_file =~ /\_?(?:html)?\.dyn$/)
      ...
      if cfg["layout"]
        if cfg["layout"][0] == "~" #user mode
          ...
          cfg_tmp=File.join(dyn_root,"system","layout",cfg["layout"][8..-1])
        else
      ...
      if cfg["pre"]
        if cfg["pre"][0] == "~" #user mode
          ...
          cfg_tmp=File.join(dyn_root,"system","preload",cfg["pre"][8..-1])
        else
      ...
      if cfg["post"]
        if cfg["post"][0] == "~" #user mode
          ...
          cfg_tmp=File.join(dyn_root,"system","postload",cfg["post"][8..-1])
        else
